...
/**
 * A JukeBox for sampled and midi sound files. Features duration progress, seek slider, pan and volume controls.
 * 
		...
		if (currentSound instanceof AudioInputStream) {
			try {
				...
				if ((format.getEncoding() == AudioFormat.Encoding.ULAW) || (format.getEncoding() == AudioFormat.Encoding.ALAW)) {
					AudioFormat tmp = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, format.getSampleRate(), format.getSampleSizeInBits() * 2,
		...
		if (currentSound instanceof Sequence || currentSound instanceof BufferedInputStream && thread != null) {
			...
			while ((paused || clip.isActive()) && thread != null && !bump) {
				try {
					...
					long ms = clip.getMicrosecondPosition();
					System.out.println("Micro: " + ms);
					double seconds = clip.getFramePosition() / clip.getFormat().getFrameRate();
					System.out.println("SECONDS: " + seconds + " - getSeconds = " + getSeconds());
				} catch (Exception e) {
					break;
				}
			}
